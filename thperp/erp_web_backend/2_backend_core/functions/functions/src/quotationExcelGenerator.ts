  ...
  .https.onCall(
    async (
      ...
      try {
        ...
        const fileName = `quotation-pdfs/${projectId}/${quotationNumber}.xlsx`;
        ...
        // Generate signed URL instead of makePublic() to avoid IAM policy permission issues
        // Signed URL valid for 10 years
        const [signedUrl] = await file.getSignedUrl({
          action: 'read',
          expires: '03-17-2035', // 10 years from now
        });

        functions.logger.info('Đã tạo file Excel thành công:', { excelUrl: signedUrl });
        return { excelUrl: signedUrl };
      } catch (error: any) {
